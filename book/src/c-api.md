...
The `NSI_ALL_NODES` macro defines a special handle to refer to all nodes in some contexts, such as [removing connections](#severing-connections).

```c
#define NSI_ALL_ATTRIBUTES ".all"
```

The `NSI_ALL_ATTRIBUTES` macro defines a special handle to refer to all attributes in some contexts, such as [removing connections](#severing-connections).
...
Attributes are properties of nodes and are only set _through_ the aforementioned optional arguments using the `NSISetAttribute()` and `NSISetAttributeAtTime()` functions.
...
| `NSITypeHPoint`       | Homogeneous point `(w·x, w·y, w·z, w)`, given as four 32-bit floating point values. The stream spells it `hpoint`. See [Geometry in the Type System](design/geometry-types.md) for the draft name `weighted-point`. |
