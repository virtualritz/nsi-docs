# Summary

# General

- [Background](background.md)

# The API

- [The Interface](interface.md)
  - [The C API](c-api.md)
  - [The C++ API](cpp-api.md)
  - [The Rust API](rust-api.md)
  - [The Lua API](lua-api.md)
  - [The Python API](python-api.md)
  - [The Stream API](stream-api.md)
  - [Procedurals](procedurals.md)
- [Nodes](nodes.md)
  - [Common Attributes](nodes/common-attributes.md)
  - [`root`](nodes/root.md)
  - [`global`](nodes/global.md)
  - [`set`](nodes/set.md)
  - [`plane`](nodes/plane.md)
  - [`mesh`](nodes/mesh.md)
  - [`nurbs`](nodes/nurbs.md)
  - [`t-nurcc`](nodes/t-nurcc.md)
  - [`faceset`](nodes/faceset.md)
  - [`curves`](nodes/curves.md)
  - [`nurbs-curves` (draft)](nodes/nurbs-curves.md)
  - [`particles`](nodes/particles.md)
  - [`procedural`](nodes/procedural.md)
  - [`environment`](nodes/environment.md)
  - [`shader`](nodes/shader.md)
  - [`attributes`](nodes/attributes.md)
  - [`transform`](nodes/transform.md)
  - [`instances`](nodes/instances.md)
  - [`outputdriver`](nodes/outputdriver.md)
  - [`outputlayer`](nodes/outputlayer.md)
  - [`screen`](nodes/screen.md)
  - [`vdbparticles`](nodes/vdbparticles.md)
  - [`volume`](nodes/volume.md)
  - [Camera Nodes](nodes/camera-nodes.md)
    - [`orthographiccamera`](nodes/orthographiccamera.md)
    - [`perspectivecamera`](nodes/perspectivecamera.md)
    - [`fisheyecamera`](nodes/fisheyecamera.md)
    - [`cylindricalcamera`](nodes/cylindricalcamera.md)
    - [`sphericalcamera`](nodes/sphericalcamera.md)
    - [Lens Shaders](nodes/lens-shaders.md)
- [Script Objects](script_objects.md)

# How To

- [Rendering Guidelines](guidelines.md)
- [Cookbook](cookbook.md)
- [Implementer FAQ](faq.md)

# Implementations

- [Implementations](implementations.md)
  - [3Delight](implementations/3delight.md)
    - [ᴏsʟ Extensions](osl-extensions.md)
  - [MoonRay](implementations/moonray.md)

# Design Drafts

- [NURBS: Draft Design](design/nurbs-draft.md)
- [Node Types: Naming and Granularity](design/node-types.md)
- [Trim Curves: API Alternatives](design/trim-curves.md)
  - [Option 1: Inline Attributes](design/trim-curves-inline.md)
  - [Option 2: `trim` Nodes](design/trim-curves-nodes.md)
  - [Retired: `edge` Nodes](design/trim-curves-edges.md)
- [Shared Boundaries: Weld Declarations](design/shared-boundaries.md)
- [NURBS Curves and Point Welds](design/curve-point-welds.md)
- [Geometry in the Type System](design/geometry-types.md)
- [Type Names: API Alternatives](design/type-names.md)
  - [Encoding 1: Composite Constants](design/type-names-composite.md)
  - [Encoding 2: A Named Bit](design/type-names-named-bit.md)
  - [Encoding 3: A Flag in `flags`](design/type-names-flag.md)
  - [Decision 2: Vocabulary](design/type-names-vocabulary.md)
  - [Decision 3: Scope and Contract](design/type-names-scope.md)
  - [Type Rename: Role, Components, Width](design/type-rename.md)

# Appendices

- [Acknowledgments](acknowledgments.md)
- [`nsi.h`](nsi.h.md)
- [`nsi.hpp`](nsi.hpp.md)
- [`nsi_dynamic.hpp`](nsi_dynamic.hpp.md)
- [`nsi.py`](nsi.py.md)
- [`nsi_procedural.h`](nsi_procedural.h.md)
- [`gear.cpp`](gear.cpp.md)
- [Naming Convention Redesign](naming-convention.md)
