...
When the node exists, its curve is authoritative: where a face's pcurve (mapped through its surface) and the edge curve disagree, the edge curve defines the true boundary position; pcurves define the parametric locus and the trimmed-region classification.
...
- The heaviest option: exporters emit a second curve representation (3D edge curves in addition to pcurves), and roughly one edge node per two face-uses — edge nodes outnumber faces in typical solids.
...
- Two sources of truth for the boundary (pcurve vs edge curve) require the precedence rule above, and tolerance disagreements between them become the exporter's bug to own.
