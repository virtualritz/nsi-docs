...
- **Interactive Rendering and Scene Edits** — Scene edit operations should not be a special case. There should be no difference between scene _description_ and scene _edits_. In other words, a scene description is a series of edits and vice versa.
- **Tight Integration with Open Shading Language** — [ᴏsʟ](https://opensource.imageworks.com/?p=osl) integration is not superficial and affects scene definition. For example, there are no explicit light sources in ɴsɪ: light sources are created by connecting shaders with an `emission()` closure to a geometry.
